...
print(f"[2] Features:        {df_f.shape[1]} cols, NaN={nan_pct:.1%} [OK]")
...
print(f"[3] Regime:          {cur['regime_name']} (conf={cur['confidence']:.2f}) [OK]")
...
print(f"[4] Signal Engine:   {total_sigs} signals in {n} bar walk-forward [OK]")
...
print(f"[5] ML Filter:       trained (thr={mlf.optimal_threshold:.2f} pf={mlf._holdout_pf:.2f})  predict=pass={pred['pass']} prob={pred['probability']} [OK]")

# Cold-start pretrain
mlf2 = MLSignalFilter(min_samples=30)
mlf2.pretrain_from_backtest(trades_df, df_f)
print(f"[5b] ML Pretrain:    source={mlf2._source} thr={mlf2.optimal_threshold:.2f} [OK]")
...
print(f"[6] Exec Analytics:  slippage={slip:.4%} [OK]")

# ─── 8. Risk Manager ──────────────────────────────────────────────────────────
from core.risk_manager import RiskManager
rm = RiskManager()
print(f"[7] Risk Manager:    capital=${rm.capital:.0f} [OK]")
...
print(f"[8] Market Structure: score={float(ms_scores.iloc[-1]):.2f}  boosted_conf={boosted:.3f} [OK]")
...
print(f"[9] OI Factor:       score range=[{float(oi_score.min()):.3f}, {float(oi_score.max()):.3f}] [OK]")

print()
print("════════════════════════════════════════")
print("  ALL SYSTEMS GO [OK]  TradingClaw v5 OK  ")
print("════════════════════════════════════════")
